/*
 * Fuzzy Clock Calendar applet for Cinnamon DE fuzzyclock@mroberts.me
 * Copyright (c) 2016 Matthew Roberts <contact@mroberts.me>
 * 
 * Shows a fuzzy clock instead of exact time.
 * version 0.1
 * 
 * Based on the Cinnamon calendar applet, Copyright (c) its respective
 * authors.
 * 
 * Portions originate from the Fuzzy clock Conky script
 * Copyright (c) cumpstey
 * https://www.gnome-look.org/content/show.php/Fuzzy+clock?content=54716
 * 
...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height, instance_id) {        
        ...
        try {    
            ...
            this.settings = new Settings.AppletSettings(this, "fuzzyclock@mroberts.me", this.instance_id);
    ...
    _updateClockAndDate: function() {
        ...
        var Hours = h;
        var FuzzyMinute = 5 * (Math.round((m + (s / 60.0)) / 5, 0));
        if(FuzzyMinute>30){
            Hours = Hours + 1;
        }
        ...
        var TextMins = ['five', 'ten', 'quarter', 'twenty', 'twenty five', 'half', 'twenty five', 'twenty', 'quarter', 'ten', 'five', 'nothing'];
        var TextMin = TextMins[parseInt(FuzzyMinute / 5 -1)];
        var TextHours = ['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'eleven', 'twelve'];
        var TextHour = TextHours[parseInt(Hours - 1)];

        var label_string;
        if(FuzzyMinute == 0) {
            label_string = TextHour + TextTime + TimeOfDay;
        }else{
            label_string = TextMin + TextTime + TextHour + TimeOfDay;
        }
